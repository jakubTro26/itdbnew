...
if ( class_exists( 'WPBakeryShortCode' ) && ! class_exists( 'Radiantthemes_Style_Button' ) ) {
	...
	class Radiantthemes_Style_Button extends WPBakeryShortCode {
		...
		public function radiantthemes_button_style_func( $atts, $content = null, $tag ) {
			...
			$output = "\r" . '<!-- rt-button -->' . "\r";

			$output .= '<div class="rt-button element-one hover-style-' . esc_attr( $shortcode['button_hover_style'] ) . $animation_classes . ' ' . esc_attr( $shortcode['extra_class'] ) . '" ' . $button_id . ' data-button-direction="' . $shortcode['align'] . '"';
			...
			if ( strlen( $url['url'] ) > 0 ) {
				$output .= '<a style="' . $stylecolor . '" class="rt-button-main" href="' . esc_attr( $url['url'] ) . '" ' . $rel . ' title="' . esc_attr( $url['title'] ) . '" target="' . ( strlen( $url['target'] ) > 0 ? esc_attr( $url['target'] ) : '_self' ) . '">';
