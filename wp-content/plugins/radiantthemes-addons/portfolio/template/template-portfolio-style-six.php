...
$output  = '<div class="rt-portfolio-box-filter element-six filter-style-' . esc_attr( $shortcode['portfolio_filter_style'] ) . ' text-' . esc_attr( $shortcode['portfolio_filter_alignment'] ) . ' ' . esc_attr( $hidden_filter ) . '">';
...
$output .= '</div>';
$output .= '<div class="rt-portfolio-box element-six row isotope ' . esc_attr( $enable_zoom ) . '" data-portfolio-box-align="' . esc_attr( $shortcode['portfolio_box_alignment'] ) . '" style="margin-left:-' . esc_attr( $spacing_value ) . 'px; margin-right:-' . esc_attr( $spacing_value ) . 'px;">';
...
if ( $my_query->have_posts() ) {
	while ( $my_query->have_posts() ) :
		...
		$output .= '<img src="' . plugins_url( 'radiantthemes-addons/portfolio/images/blank-image-100x80.jpg' ) . '" alt="blank image" width="100" height="80">';
		$output .= '<div class="holder" style="background-image:url('. get_the_post_thumbnail_url( get_the_ID(), 'full' ) .')">';
		$output .= '</div>';
