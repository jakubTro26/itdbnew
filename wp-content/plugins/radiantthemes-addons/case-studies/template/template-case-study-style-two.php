...
$output  = '<div class="rt-case-study-box-filter element-two filter-style-' . esc_attr( $shortcode['case_study_filter_style'] ) . ' text-' . esc_attr( $shortcode['case_study_filter_alignment'] ) . ' ' . esc_attr( $hidden_filter ) . '">';
...
$output .= '</div>';
$output .= '<div class="rt-case-study-box element-two row isotope ' . esc_attr( $enable_zoom ) . '" data-case-study-box-align="' . esc_attr( $shortcode['case_study_box_alignment'] ) . '" style="margin-left:-' . esc_attr( $spacing_value ) . 'px; margin-right:-' . esc_attr( $spacing_value ) . 'px;">';
	...
	while ( $my_query->have_posts() ) :
		...
		if ( true == $shortcode['case_study_enable_title'] ) {
		    if ( 'yes' === $shortcode['case_study_enable_zoom'] ) {
    		    $output .= '';
    		    $output .= '<h4><a class="fancybox" href="' . get_the_post_thumbnail_url( get_the_ID(), 'full' ) . '">' . get_the_title() . '</a></h4>';
    		} else {
